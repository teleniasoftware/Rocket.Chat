...
API.v1.addRoute(
  ...
  { authRequired: true },
  {
    get() {
      ...
      let result;
      Meteor.runAsUser(this.userId, () => {
        result = Meteor.call('messageSearch', searchText, roomId, count).message
          .docs;
      });
...
API.v1.addRoute(
  ...
  { authRequired: true },
  {
    post() {
      ...
      const sent = Meteor.runAsUser(this.userId, () =>
        Meteor.call('sendMessage', this.bodyParams.message)
      );
...
API.v1.addRoute(
  ...
  { authRequired: true },
  {
    post() {
      ...
      Meteor.runAsUser(this.userId, () =>
        Meteor.call('setReaction', emoji, msg._id, this.bodyParams.shouldReact)
      );
  ...
  { authRequired: true },
  {
    post() {
      ...
      if (!roomId) {
        throw new Meteor.Error(
          'error-invalid-params',
          'The required "roomId" param is missing.'
        );
      }

      Messages.removeByRoomId(roomId);
      Subscriptions.removeByRoomId(roomId);
      const res = LivechatRooms.removeById(roomId);
      return API.v1.success(res);
    },
  }
);
